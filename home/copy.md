# Copy - Home (Ecossistema Clube do Leilão)

> Blocos globais (nav, footer, faixa de confiança, contatos, placeholders de URL): ver `/_shared/copy-global.md`
> Fonte de design: `design-kit/project/Ecossistema/Home.dc.html`

## Hero

- Badge (link para /evento/): PRÓXIMO EVENTO AO VIVO · [DATA_EVENTO] · [CIDADE_EVENTO]
- Headline: Conquiste seu imóvel pagando menos, com segurança jurídica.
  (destaque em itálico/cor no trecho "segurança jurídica.")
- Subheadline: Do arremate à escritura: o ecossistema completo da Clube do Leilão reúne leilão, financiamento, despachante, cursos e imóveis em um só lugar.
- CTA primário: Garantir minha vaga → /evento/
- CTA secundário: Conhecer o ecossistema → âncora para a seção Pilares
- Visual: imagem premium (chave na mão / evento ao vivo) sobre fundo escuro, com card flutuante:
  - Eyebrow: LEILÃO & PROSA
  - Título: Encontro presencial
  - Apoio: Vagas limitadas · cadeira garantida

### Métricas do hero
- 42% — desconto médio no arremate
- +2.400 — alunos formados
- 100% — acesso liberado na hora

## Seção: Faixa de confiança

- Ver bloco em `/_shared/copy-global.md` (Asaas · Digital Manager Guru · MemberKit · Caixa)

## Seção: Pilares (O Ecossistema)

- Eyebrow: O ECOSSISTEMA
- Título: Tudo o que envolve morar, em um só lugar.
- Link auxiliar: Ver todos os serviços → /servicos/

Cards (grade 3x2, numerados):
1. Clube do Leilão → /clube-do-leilao/
   Arremate com mapa real e a calculadora que blinda o seu lance.
2. Cursos e Mentorias → /cursos/
   Aprenda a arrematar do zero, com quem já fez na prática.
3. Financiamento e Crédito → /financiamento/
   Análise e assessoria para financiar seu imóvel com a Caixa.
4. Despachante e Imóveis → /despachante/
   Documentação, posse e imóveis particulares sem corretor.
5. Consórcio e Seguros → /consorcio/
   Consórcio Caixa, veículos e seguros de vida e residencial.
6. Marketplace → /servicos/
   Imóveis selecionados, prontos para morar ou investir.

## Seção: Como funciona (fundo escuro)

- Eyebrow: DO LEILÃO À CHAVE NA MÃO
- Título: Um caminho claro, do primeiro lance à escritura.

Passos (4 colunas):
- Passo 01 — Aprenda e escolha: Curso, mentoria e a calculadora para achar a oportunidade certa.
- Passo 02 — Arremate com segurança: Análise jurídica e mapa real do imóvel antes de dar o lance.
- Passo 03 — Financie e regularize: Crédito na Caixa, despachante e toda a documentação resolvida.
- Passo 04 — Receba a chave: Posse, escritura e o imóvel pronto para morar ou investir.

## Seção: Imóveis em destaque

- Eyebrow: EM DESTAQUE
- Título: Oportunidades abertas agora
- CTA auxiliar: Ver marketplace completo → /servicos/

Cards [PLACEHOLDER — dados de exemplo do design; substituir por imóveis reais antes de publicar]:
1. BELO HORIZONTE · MG — Apartamento 2 quartos · Buritis — Lance inicial R$ 198.000 — selo −48% — 1ª praça
2. CONTAGEM · MG — Casa 3 quartos · Eldorado — Lance inicial R$ 305.000 — selo −35% — Aberto
3. NOVA LIMA · MG — Cobertura · Vila da Serra — Lance inicial R$ 690.000 — selo −52% — 2ª praça

## Seção: Teaser Cursos + Marketplace (2 cards lado a lado)

Card 1 (claro):
- Eyebrow: CURSOS & MENTORIAS
- Título: Aprenda a arrematar do zero — e acesse na hora.
- Texto: Pagou, acesso liberado automaticamente na área de membros. Sem espera, sem trabalho manual.
- CTA: Ver trilhas de curso → /cursos/

Card 2 (verde escuro):
- Eyebrow: MARKETPLACE
- Título: Imóveis particulares, sem corretor.
- Texto: Compre direto, com consultoria da Clube do Leilão e toda a parte documental cuidada por nós.
- CTA: Explorar imóveis → /consultoria-sem-corretor/

## Seção: Banda do evento (fundo escuro)

- Eyebrow: AO VIVO · BÔNUS DO ECOSSISTEMA
- Título: Leilão & Prosa — [DATA_EVENTO], [CIDADE_EVENTO]
- Texto: Um encontro presencial e estratégico para arrematar sem cair em pegadinhas. Vagas limitadas.
- CTA: Garantir cadeira → /evento/

## Seção: Footer

- Versão completa — ver `/_shared/copy-global.md`

## [OPCIONAL — fora do design, não implementar sem aprovação]

FAQ sugerida como matéria-prima futura:
- Leilão de imóvel é seguro? Como o Clube reduz o risco jurídico?
- Preciso ter todo o valor à vista para arrematar?
- Consigo financiar um imóvel de leilão pela Caixa?
- O acesso ao curso é liberado imediatamente após o pagamento?

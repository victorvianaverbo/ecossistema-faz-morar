# Copy - Corretagem de Imóveis Particulares

> Blocos globais: ver `/_shared/copy-global.md` · Template visual: `design-kit/project/Ecossistema/ServicoLayout.dc.html`
> Fonte de design: `design-kit/project/Ecossistema/Corretagem.dc.html`

## Breadcrumb
Início (/) / Serviços (/servicos/) / Corretagem de Imóveis Particulares

## Hero
- Eyebrow (categoria): CORRETAGEM
- Headline: Compre o imóvel particular certo, bem assessorado.
- Descrição: Corretagem para compra de imóveis particulares com a curadoria e a segurança da Faz Morar — negociação, análise e documentação em um só lugar.
- CTA primário: Buscar meu imóvel → https://wa.me/5531996951660 (mensagem pré-preenchida: "Olá, quero ajuda para encontrar um imóvel")
- CTA secundário: Falar no WhatsApp → https://wa.me/5531996951660
- Card de fato (painel escuro):
  - Label: CURADORIA
  - Valor: Selecionada
  - Nota: imóveis avaliados por quem conhece o mercado de perto.

## Seção: O que está incluído
- Título fixo: Cada detalhe resolvido por nós.
1. Curadoria de imóveis — Seleção de oportunidades particulares alinhadas ao seu perfil.
2. Negociação — Conduzimos a proposta buscando o melhor preço e condição.
3. Análise do imóvel — Checagem de documentação e situação antes da compra.
4. Fechamento seguro — Contrato, pagamento e transferência acompanhados por nós.

## Seção: Como funciona (fundo escuro)
- Título fixo: Simples, do primeiro contato ao resultado.
- Passo 01 — Diga o que procura: Localização, perfil e orçamento do imóvel dos sonhos.
- Passo 02 — Receba opções: Apresentamos imóveis selecionados para você.
- Passo 03 — Feche com segurança: Cuidamos da negociação e da documentação.

## Seção: Nota + CTA final
- Título fixo: Pronto para dar o próximo passo?
- Nota: Você compra com um corretor de confiança e todo o suporte documental da Faz Morar.
- Card: Corretagem de Imóveis Particulares · CTA "Buscar meu imóvel" → WhatsApp · WhatsApp (31) 99695-1660

## Seção: Footer
- Versão compacta — ver `/_shared/copy-global.md`

## [OPCIONAL — fora do design, não implementar sem aprovação]
- Qual a diferença entre a corretagem e a consultoria sem corretor?
- Vocês também vendem meu imóvel atual?
- A comissão é paga por quem compra ou por quem vende?
